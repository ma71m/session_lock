...
public class DeviceAppsPlugin implements
    ...
    private List<Map<String, Object>> getInstalledApps(boolean includeSystemApps, boolean includeAppIcons, boolean onlyAppsWithLaunchIntent) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            apps = packageManager.getInstalledPackages(PackageManager.PackageInfoFlags.of(0));
        } else {
            apps = packageManager.getInstalledPackages(0);
        }
    ...
    private Map<String, Object> getAppData(PackageManager packageManager,
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
            map.put(AppDataConstants.VERSION_CODE, pInfo.getLongVersionCode());
        } else {
            map.put(AppDataConstants.VERSION_CODE, (long) pInfo.versionCode);
        }
